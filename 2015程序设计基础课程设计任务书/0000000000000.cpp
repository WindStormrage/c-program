...
//	int payment2(char );//补交功能(共5000,显示欠费和已交费) 
	     ...
	     if(man!=1&&man!=2&&man!=0)
		 ...
		 else if(man==2)
		   {
			...
			for(a=0;a==0;) 
		   	   {
			    printf("+================================+\n");
			    printf("|1.报到服务 2.缴费服务           |\n");
		     	printf("|                                |\n");
		        printf("|3.查询服务 4.排序服务 5.返回上层|\n");
		    	printf("|                                |\n");
		        printf("|      请输入相应的服务编号      |\n");    
		        printf("+================================+\n");
		        scanf ("%d",&x); 
    ...
    int add(int count)
	{
		...
		for(int i=0;i<count;i++)
		   fprintf(fp,"%s %s %d %d %d %d %d\n",stu[i].name,stu[i].number,stu[i].math,stu[i].english,stu[i].chinese,stu[i].all,stu[i].money); 
	    fclose(fp);
		return count;
	}
    ...
    int old_data(int count)
    {
    	...
    	fscanf(fp,"%d",&count);
    	for(i=0;i<count;i++)
     		fscanf(fp,"%s%s%d%d%d%d%d",stu[i].name,stu[i].number,&stu[i].math,&stu[i].english,&stu[i].chinese,&stu[i].all,&stu[i].money); 
		fclose(fp);
    	return count;    	
	}
	...
	{  
	 ...
	 for(i=0;i<count;i++)
	 	{
	 		if(strcmp(zh,stu[i].number)==0&&strcmp(mm,stu[i].number)==0)
                t=1;
	 	}
	 return t;
	}
